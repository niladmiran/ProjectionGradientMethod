...
def proj_lpball(y, p, x, radius=1, method='bisection'):
    """
    ...
    while True:
        ...
        x_opt, lamb = proj_weightedl1ball(np.abs(y),
                                          weight,
                                          radius_k,
                                          lamb,
                                          method)
        ...
        condition = (LA.norm(x_real - x) * LA.norm(weight, 2) ** tau <= M)

        error = abs(LA.norm(x_real, p) ** p - radius)

        # update epsilon
        if condition and min(epsilon) >= 1e-20:
            epsilon *= np.minimum(error, 1 / np.sqrt(iters))
...
def hardThreshold(x, k):
    ...
    ----------
    x: array of shape (n, )
        the input of the hard thres hold
    k: interger
        threshold

    Returns
    ---------
    x: the output of the hard threshold
    """
    sorted_x = sorted(np.abs(x), reverse=True)
